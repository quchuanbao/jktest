...
class ResultslogController extends Controller 
{
	...
	{
		$data['name'] = "业绩表";
		$model = new ResultslogForm();
		foreach ($model->attributes as $n => $v) {
			if (!empty($_REQUEST[$n])) {
				$model->$n = $_REQUEST[$n];
			}
		}
		$model->attributes = $_POST['ResultslogForm'];
		foreach ($model->attributes as $n => $v) {
			if(!empty($v)){
				$pageInfo.=$n."/".$v;
			}
		...
		$from = ($p-1)*$limit;
		$page_nums = 10;
		$page = new sdkPage($total,$p,$limit,$page_nums,"/mywork/resultslog/index/".$pageInfo."page/");
	...
	public function actionAdd()
	{
		...
		if (isset($_POST['ResultslogForm'])) {
			...
			if($model->validate()){
				...
				$this->showmsg("操作成功",'/mywork/resultslog');
			}
		}
		$data['model'] = $model;
		$data['options'] = $model->getOptionName();
