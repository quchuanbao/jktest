...
class ComplaintlogController extends Controller 
{
	...
	{
		$data['name'] = "投诉处理表";
		$model = new ComplaintlogForm();
		foreach ($model->attributes as $n => $v) {
			if (!empty($_REQUEST[$n])) {
				$model->$n = $_REQUEST[$n];
			}
		}
		$model->attributes = $_POST['ComplaintlogForm'];
		foreach ($model->attributes as $n => $v) {
			if(!empty($v)){
				$pageInfo.=$n."/".$v;
			}
		...
		$dataModel = new linkComplaintlog();
		...
		$from = ($p-1)*$limit;
		$page_nums = 10;
		$page = new sdkPage($total,$p,$limit,$page_nums,"/mywork/complaintlog/index/".$pageInfo."page/");
	...
	{
		$model = new ComplaintlogForm();
		if (isset($_POST['ComplaintlogForm'])) {
			$model->attributes = $_POST['ComplaintlogForm'];
			if($model->validate()){
				$dataModel = new linkComplaintlog();
				$dataModel->initVar($dataModel);
				...
				$this->showmsg("操作成功",'/mywork/complaintlog');
			}
		}
		$data['model'] = $model;
		$data['options'] = $model->getOptionName();
