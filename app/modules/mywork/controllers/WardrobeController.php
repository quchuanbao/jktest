...
class WardrobeController extends Controller 
{
	...
	{
		$data['name'] = "衣柜租用";
		$model = new WardrobeForm();
		foreach ($model->attributes as $n => $v) {
			if (!empty($_REQUEST[$n])) {
				$model->$n = $_REQUEST[$n];
			}
		}
		$model->attributes = $_POST['WardrobeForm'];
		foreach ($model->attributes as $n => $v) {
			if(!empty($v)){
				$pageInfo.=$n."/".$v;
			}
		...
		$from = ($p-1)*$limit;
		$page_nums = 10;
		$page = new sdkPage($total,$p,$limit,$page_nums,"/mywork/wardrobe/index/".$pageInfo."page/");
	...
	public function actionAdd()
	{
		...
		if (isset($_POST['WardrobeForm'])) {
			...
			if($model->validate()){
				...
				$this->showmsg("操作成功",'/mywork/wardrobe');
			}
		}
		$data['model'] = $model;
		$data['options'] = $model->getOptionName();
