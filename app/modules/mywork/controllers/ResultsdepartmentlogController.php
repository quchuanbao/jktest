...
class ResultsdepartmentlogController extends Controller 
{
	public $layout='main';
	/**
	 * 显示部门业绩表 
	...
	{
		$data['name'] = "部门业绩表";
		$model = new ResultsdepartmentlogForm();
		foreach ($model->attributes as $n => $v) {
			if (!empty($_REQUEST[$n])) {
				$model->$n = $_REQUEST[$n];
			}
		}
		$model->attributes = $_POST['ResultsdepartmentlogForm'];
		foreach ($model->attributes as $n => $v) {
			if(!empty($v)){
				$pageInfo.=$n."/".$v;
			}
		...
		$dataModel = new linkResultsdepartmentlog();
		...
		$from = ($p-1)*$limit;
		$page_nums = 10;
		$page = new sdkPage($total,$p,$limit,$page_nums,"/mywork/resultsdepartmentlog/index/".$pageInfo."page/");
	...
	{
		$model = new ResultsdepartmentlogForm();
		if (isset($_POST['ResultsdepartmentlogForm'])) {
			$model->attributes = $_POST['ResultsdepartmentlogForm'];
			if($model->validate()){
				$dataModel = new linkResultsdepartmentlog();
				$dataModel->initVar($dataModel);
				...
				$this->showmsg("操作成功",'/mywork/resultsdepartmentlog');
			}
		}
		$data['model'] = $model;
		$data['options'] = $model->getOptionName();
